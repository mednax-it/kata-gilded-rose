...
class GildedRoseTest(TestCase):
    ...
    def test_quality_goes_up_by_one_for_improving_products_with_11_days_left(self):
        self.items.append(Item("Aged Brie", 11, 34))
        self.items.append(
            Item("Backstage passes to a TAFKAL80ETC concert", 11, 30))
        gilded_rose.update_item(self.items)
        expected = [
            {'sell_in': 10, 'quality': 35},
            {'sell_in': 10, 'quality': 31},
        ]
